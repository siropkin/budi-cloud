...
- **Run prettier before every commit**: CI's `format` job (`.github/workflows/ci.yml`) runs `npm run format:check` → `prettier --check .` and fails the PR on any drift. Run `npm run format` (or `npx prettier --write <files>`) and re-stage before `git commit` — even small copy-only edits can trigger reflows that fail the check. If you've already committed, add a follow-up `style: prettier --write …` commit rather than amending.
...
- **Migrations ship via CI, not by hand**: `.github/workflows/db-push.yml` runs `supabase db push --include-all` against the linked production project on every push to `main` that touches `supabase/migrations/**`. PRs that add a migration are dry-run twice: against a fresh Postgres in `.github/workflows/ci.yml` (catches syntax/dependency errors) and against the live prod schema in `.github/workflows/db-shadow.yml` (catches prod-specific drift — see the 2026-05-15 incident, where migration 003 was recorded as applied on prod but its DDL never ran, and that latent drift broke migration 025 on merge). Do **not** apply migrations through the Supabase SQL editor — that re-introduces drift of the same shape (#92/#94). If you must hand-apply (incident recovery), run `supabase migration repair` afterwards so `supabase migration list` shows `Local == Remote` again. Required GitHub Actions secrets: `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD`, `SUPABASE_PROJECT_REF`.

- **Schema and code renames ship in separate PRs, schema first**: a column rename in one commit alongside code that already reads the new name leaves prod broken for the gap between Vercel deploy and `db-push` completion. Always: (PR 1) additive migration that exposes the new name _and_ keeps the old name working (view, generated column, or compat shim); (PR 2) code switches to the new name; (PR 3) drop the old name. The 2026-05-15 incident hit because `db-push` failed mid-rename and Vercel deployed the renamed-code half anyway — every authenticated request hit a column that no longer existed on prod.
